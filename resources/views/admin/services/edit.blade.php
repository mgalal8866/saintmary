        ...
        <form method="POST" action="{{ route("admin.services.update", [$service->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('service') ? 'is-invalid' : '' }}" name="service_id" id="service_id">
                    @foreach($services as $id => $entry)
                        <option value="{{ $id }}" {{ (old('service_id') ? old('service_id') : $service->service->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            {{-- <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('subservies') ? 'is-invalid' : '' }}" name="subservies_id" id="subservies_id">
                    @foreach($subservies as $id => $entry)
                        <option value="{{ $id }}" {{ (old('subservies_id') ? old('subservies_id') : $service->subservies->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('mainservice') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="mainservice" id="mainservice" value="1" {{ $service->mainservice || old('mainservice', 0) === 1 ? 'checked' : '' }}>
